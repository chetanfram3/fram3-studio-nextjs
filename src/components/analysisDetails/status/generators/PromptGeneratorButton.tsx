...
/**
 * PromptGeneratorButton - Triggers prompt generation for scenes
 *
...
export default function PromptGeneratorButton({
  ...
  >({
    ...
    onSuccess: (response) => {
      const success =
        response.message === "Prompt Generator started in the background!";
      if (success) {
  ...
  const handleClick = () => {
    ...
    mutate({
      ...
      scriptId,
      versionId,
      analysisType: "promptGenerator",
    });
  };

  return (
    <Tooltip title="Generate Prompts" arrow>
      <span>
        <IconButton
          onClick={handleClick}
